set(ENABLE_JAKT ON CACHE BOOL "Enable jakt" FORCE)
...
set(JAKT_COMPILER "/Users/test/source/jakt/build/bin/jakt")
set(JAKT_TARGET_TRIPLE ${arch}-unknown-${os}-unknown)
set(JAKT_LIBRARY_BASE "/Users/test/source/jakt/build/lib")
set(JAKT_LIBRARY_DIR "${JAKT_LIBRARY_BASE}/${JAKT_TARGET_TRIPLE}")
set(JAKT_INCLUDE_DIR "${JAKT_LIBRARY_BASE}/../include/runtime")
...
if (ENABLE_JAKT)
    if (NOT EXISTS ${JAKT_COMPILER})
        ...
        set(all_includes
            ...
            ${LADYBIRD_SOURCE_DIR}/Libraries/LibCrypt
            ${LADYBIRD_SOURCE_DIR}/Libraries/LibSystem
        ...
        message(STATUS "Checking if Jakt compiler at ${JAKT_COMPILER} supports C++ interop (include paths: ${include_args})")
...
function(add_jakt_executable target source)
    ...
    foreach(flag IN LISTS compile_flags)
        if (flag MATCHES ".*Swift.*")
            continue()
        endif()
        if (flag MATCHES "^\\$<\\$<COMPILE_LANGUAGE:(.*)>:(.*)>$")
            string(REGEX REPLACE "^\\$<\\$<COMPILE_LANGUAGE:(.*)>:(.*)>$" "\\2" flag ${flag})
            string(REPLACE "$<SEMICOLON>" ";" flag ${flag})
            foreach(flag_part IN LISTS flag)
                if (flag_part MATCHES "^\\$<.*>$")
                    continue()
                endif()
                # If it's empty, skip it
                if (flag_part STREQUAL "")
                    continue()
                endif()
                message("Jakt: Adding extra cpp flag ${flag_part}")
                list(APPEND extra_cpp_flags "--extra-cpp-flag${flag_part}")
            endforeach()
            continue()
        endif()

        message("Jakt: Adding extra cpp flag ${flag}")
    ...
    add_custom_command(
        ...
        COMMAND ${JAKT_COMPILER}
            ...
            --extra-cpp-flag-Wno-suggest-override
            ${configs}
            ${includes}
            ${extra_cpp_sources}
            "${source_abs}"
