...
impl TransportSocket {
    ...
    pub fn post_message(&mut self, payload: &[u8], attachments: &mut Vec<Attachment>) -> io::Result<()> {
        ...
        if payload_size > MAX_MESSAGE_PAYLOAD_SIZE {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "payload exceeds MAX_MESSAGE_PAYLOAD_SIZE"));
        }
        ...
        if fd_count > MAX_MESSAGE_FD_COUNT {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "fd count exceeds MAX_MESSAGE_FD_COUNT"));
        }
    ...
    fn try_extract_message(&mut self) -> io::Result<Option<TransportMessage>> {
        loop {
            ...
            if header.fd_count > MAX_MESSAGE_FD_COUNT {
                return Err(io::Error::new(io::ErrorKind::InvalidData, "peer sent oversize fd count"));
            }
            ...
            match header.msg_type {
                HEADER_TYPE_PAYLOAD => {
                    ...
                    return Ok(Some(TransportMessage { bytes: payload, attachments }));
                }
                HEADER_TYPE_FD_ACK => {
                    if header.payload_size != 0 {
                        return Err(io::Error::new(io::ErrorKind::InvalidData, "FdAck with non-zero payload"));
                    }
...
fn recv_with_fds(fd: RawFd, buf: &mut [u8], received_fds: &mut Vec<OwnedFd>, flags: libc::c_int) -> io::Result<usize> {
    ...
    let cmsg_space =
        unsafe { libc::CMSG_SPACE((MAX_TRANSFER_FDS * mem::size_of::<RawFd>()) as libc::c_uint) as usize };
    let mut cmsg_buf = vec![0u8; cmsg_space];
