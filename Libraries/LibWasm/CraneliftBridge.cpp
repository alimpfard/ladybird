...
static constexpr size_t s_oop_code_region_min_size = 256 * KiB;
static constexpr size_t s_oop_code_bytes_per_insn = 256;
...
static ALWAYS_INLINE i32 wasm_cl_finish_call(BytecodeInterpreter& interpreter, Configuration& config, FunctionAddress address, Vector<Value, ArgumentsStaticSize>& args)
{
    ...
    // Keep the compiled function table synchronized with the current module.
    ...
    // Medium path: compiled but not Cranelift (direct-threaded).
    // CallFrameHandle saves/zeros/restores the direct call counter automatically.
    if (auto* wasm_function = instance->get_pointer<WasmFunction>(); wasm_function
        && !config.should_limit_instruction_count()
        && wasm_function->code().func().body().compiled_instructions.direct) {
        BytecodeInterpreter::CallFrameHandle handle { interpreter, config };
    ...
    // Slow path: non-compiled call (interpreter or host function).
...
i64 wasm_cl_read_local(void* config_ptr, i32 index)
{
    auto& config = *static_cast<Configuration*>(config_ptr);
    return config.local(LocalIndex(index)).to<i64>();
}

void wasm_cl_write_local(void* config_ptr, i32 index, i64 value)
{
    auto& config = *static_cast<Configuration*>(config_ptr);
    config.local(LocalIndex(index)) = Value(value);
}
...
i32 wasm_cl_call_function_0(void* interp_ptr, void* config_ptr, i32 func_index)
{
    auto& interpreter = *static_cast<BytecodeInterpreter*>(interp_ptr);
    auto& config = *static_cast<Configuration*>(config_ptr);

    auto const& functions = config.frame().module().functions();
    if (static_cast<size_t>(func_index) >= functions.size())
        return 1;

    Vector<Value, ArgumentsStaticSize> args;
    return wasm_cl_finish_call(interpreter, config, functions[func_index], args);
}

i32 wasm_cl_call_function_1(void* interp_ptr, void* config_ptr, i32 func_index, i64 arg0)
{
    auto& interpreter = *static_cast<BytecodeInterpreter*>(interp_ptr);
    auto& config = *static_cast<Configuration*>(config_ptr);
    ...
    Vector<Value, ArgumentsStaticSize> args;
    args.unchecked_append(Value(arg0));
    return wasm_cl_finish_call(interpreter, config, functions[func_index], args);
}

i32 wasm_cl_call_function_2(void* interp_ptr, void* config_ptr, i32 func_index, i64 arg0, i64 arg1)
{
    auto& interpreter = *static_cast<BytecodeInterpreter*>(interp_ptr);
    auto& config = *static_cast<Configuration*>(config_ptr);
    ...
    Vector<Value, ArgumentsStaticSize> args;
    args.unchecked_append(Value(arg0));
    args.unchecked_append(Value(arg1));
    return wasm_cl_finish_call(interpreter, config, functions[func_index], args);
}

i32 wasm_cl_call_function_3(void* interp_ptr, void* config_ptr, i32 func_index, i64 arg0, i64 arg1, i64 arg2)
{
    auto& interpreter = *static_cast<BytecodeInterpreter*>(interp_ptr);
    auto& config = *static_cast<Configuration*>(config_ptr);
    ...
    args.unchecked_append(Value(arg0));
    args.unchecked_append(Value(arg1));
    args.unchecked_append(Value(arg2));
    return wasm_cl_finish_call(interpreter, config, functions[func_index], args);
}
...
i32 wasm_cl_call_indirect(void* interp_ptr, void* config_ptr, i32 table_idx, i32 type_idx, i32 element_index)
{
    ...
    auto outcome = interpreter.call_address(config, address, addrs,
        BytecodeInterpreter::CallAddressSource::IndirectCall,
        BytecodeInterpreter::CallType::UsingStack);
...
i32 wasm_cl_memory_copy(void* interp_ptr, void* config_ptr, i32 dst_mem, i32 src_mem, i32 dst_offset, i32 src_offset, i32 count)
{
    ...
    if (count > 0)
        __builtin_memmove(dst_instance->data().data() + static_cast<u32>(dst_offset),
            src_instance->data().data() + static_cast<u32>(src_offset),
...
void wasm_cl_callrec_write(void* config_ptr, i32 index, i64 value)
{
    ...
    if (!config.call_record_base()) [[unlikely]] {
        // Call record not set up, lazily allocate.
...
static ALWAYS_INLINE i32 wasm_cl_direct_call_impl(BytecodeInterpreter& interpreter, Configuration& config,
    i32 func_index, Value* args, size_t arg_count)
{
    config.build_compiled_function_table();
    ...
    if (it == config.m_compiled_fn_table.end()) [[unlikely]] {
        ...
        return wasm_cl_finish_call(interpreter, config,
            config.frame().module().functions()[func_index], args_vec);
    }
    ...
    auto outcome = handler(interpreter, config, entry.first_insn, 0,
        bit_cast<Dispatch const*>(entry.dispatches_ptr),
        bit_cast<SourcesAndDestination const*>(entry.src_dst_ptr));
...
static RuntimeHelpers make_runtime_helpers()
{
    return RuntimeHelpers {
        .read_local = bit_cast<uintptr_t>(&wasm_cl_read_local),
        .write_local = bit_cast<uintptr_t>(&wasm_cl_write_local),
        .call_function = bit_cast<uintptr_t>(&wasm_cl_call_function),
        .call_function_0 = bit_cast<uintptr_t>(&wasm_cl_call_function_0),
        .call_function_1 = bit_cast<uintptr_t>(&wasm_cl_call_function_1),
        .call_function_2 = bit_cast<uintptr_t>(&wasm_cl_call_function_2),
        .call_function_3 = bit_cast<uintptr_t>(&wasm_cl_call_function_3),
...
static CraneliftInsn serialize_insn(Dispatch const& dispatch, SourcesAndDestination const& addr)
{
    ...
    if (opc == Instructions::i32_const.value()) {
    ...
    } else if (opc == Instructions::local_get.value()
        || opc == Instructions::local_set.value()
        || opc == Instructions::local_tee.value()) {
        out.imm1 = static_cast<i64>(insn->local_index().value());
    } else if (opc == Instructions::global_get.value()
        || opc == Instructions::global_set.value()) {
        out.imm1 = static_cast<i64>(args.get<GlobalIndex>().value());
    } else if (opc == Instructions::br.value()
        || opc == Instructions::br_if.value()) {
        auto const& br_args = args.get<Instruction::BranchArgs>();
        out.imm1 = static_cast<i64>(br_args.label.value());
    } else if (opc == Instructions::block.value()
        || opc == Instructions::loop.value()
        || opc == Instructions::if_.value()) {
        ...
        // If >8 labels, continuation instructions carry the rest.
    ...
    // Synthetic opcodes.
    constexpr u64 syn_base = Instructions::SyntheticInstructionBase.value();
    if (opc >= syn_base) {
        u64 syn_offset = opc - syn_base;
        // synthetic_call_XX (0x06-0x0d): function index
        if (syn_offset >= 0x06 && syn_offset <= 0x0d) {
            out.imm1 = static_cast<i64>(args.get<FunctionIndex>().value());
        }
        // synthetic_call_with_record_0/1 (0x12-0x13): function index
        else if (syn_offset == 0x12 || syn_offset == 0x13) {
            out.imm1 = static_cast<i64>(args.get<FunctionIndex>().value());
        }
        // synthetic_br_nostack / synthetic_br_if_nostack (0x1c/0x1d)
        else if (syn_offset == 0x1c || syn_offset == 0x1d) {
            auto const& br_args = args.get<Instruction::BranchArgs>();
            out.imm1 = static_cast<i64>(br_args.label.value());
        }
        // synthetic_local_copy (0x26): local_index() = source, args = destination
        else if (syn_offset == 0x26) {
            out.imm1 = static_cast<i64>(insn->local_index().value());
            out.imm2 = static_cast<i64>(args.get<LocalIndex>().value());
        }
        // i32/i64 2local fused ops: local_index() = first, args.get<LocalIndex>() = second
        // 0x00 = i32_add2local, 0x27-0x2e = i32 sub/mul/and/or/xor/shl/shru/shrs 2local
        // 0x2f = i64_add2local, 0x33-0x3a = i64 sub/mul/and/or/xor/shl/shru/shrs 2local
        else if (syn_offset == 0x00 || (syn_offset >= 0x27 && syn_offset <= 0x2e)
            || syn_offset == 0x2f || (syn_offset >= 0x33 && syn_offset <= 0x3a)) {
            out.imm1 = static_cast<i64>(insn->local_index().value());
            out.imm2 = static_cast<i64>(args.get<LocalIndex>().value());
        }
        // i32/i64_addconstlocal, andconstlocal: local_index() = local, args = constant
        // 0x01 = i32_addconstlocal, 0x02 = i32_andconstlocal
        // 0x30 = i64_addconstlocal, 0x31 = i64_andconstlocal
        else if (syn_offset == 0x01 || syn_offset == 0x02) {
            out.imm1 = static_cast<i64>(args.get<i32>());
            out.imm2 = static_cast<i64>(insn->local_index().value());
        } else if (syn_offset == 0x30 || syn_offset == 0x31) {
            out.imm1 = args.get<i64>();
            out.imm2 = static_cast<i64>(insn->local_index().value());
        }
        // i32_storelocal (0x03), i64_storelocal (0x32): local_index() = local, args = MemoryArgument
        else if (syn_offset == 0x03 || syn_offset == 0x32) {
            auto const& mem_arg = args.get<Instruction::MemoryArgument>();
            out.imm1 = static_cast<i64>(mem_arg.offset);
            out.imm2 = static_cast<i64>(insn->local_index().value());
            out.imm3 = static_cast<u32>(mem_arg.memory_index.value()) | (mem_arg.memory_index.value() == 0 ? (1u << 31) : 0);
        }
        // local_seti32_const (0x05): local_index() = local, args = i32 constant
        else if (syn_offset == 0x05) {
            out.imm1 = static_cast<i64>(args.get<i32>());
            out.imm2 = static_cast<i64>(insn->local_index().value());
        }
        // local_seti64_const (0x3b): local_index() = local, args = i64 constant
        else if (syn_offset == 0x3b) {
            out.imm1 = args.get<i64>();
            out.imm2 = static_cast<i64>(insn->local_index().value());
        }
        // argument_get/set/tee (0x0f-0x11): local_index() = index
        else if (syn_offset >= 0x0f && syn_offset <= 0x11) {
            out.imm1 = static_cast<i64>(insn->local_index().value());
        }
    }
...
static void try_cranelift_compile_batch(Vector<BatchInput>& batch)
{
    ...
    // Compute total instruction bytes and per-function offsets.
    ...
    auto const code_region_size = max(s_oop_code_region_min_size, total_insn_count * s_oop_code_bytes_per_insn);
    auto const total_size = code_region_start + sizeof(OutputFunctionEntry) * function_count + code_region_size;

    char temporary_path[] = "/tmp/ladybird-cranelift-XXXXXX";
    int fd = mkstemp(temporary_path);
    if (fd < 0)
        return;
    ScopeGuard close_fd = [fd] { close(fd); };
    ScopeGuard unlink_temporary = [&] { unlink(temporary_path); };
    ...
    *header = InputHeader {
    ...
    // Write per-function entries and instruction data.
    ...
    for (size_t i = 0; i < function_count; ++i) {
        ...
        auto* jit_mapping = mmap(nullptr, rx_aligned_size, PROT_READ | PROT_WRITE | PROT_EXEC,
...
bool try_cranelift_compile(CompiledInstructions& compiled, u32 result_arity)
{
    ...
    // Debug knobs for bisecting miscompiles (per-process, read once):
    ...
    static auto const s_read_size_env = [](char const* name, size_t fallback) {
        if (auto* env = getenv(name))
            return static_cast<size_t>(atol(env));
        return fallback;
    };
    static auto const s_read_set_env = [](char const* name) {
    ...
    static size_t s_max_insns = s_read_size_env("CRANELIFT_MAX_INSNS", NumericLimits<size_t>::max());
    static size_t s_min_insns = s_read_size_env("CRANELIFT_MIN_INSNS", 0);
    static size_t s_min_fn = s_read_size_env("CRANELIFT_MIN_FN", 0);
    static size_t s_max_fn = s_read_size_env("CRANELIFT_MAX_FN", NumericLimits<size_t>::max());
    static auto s_skip_fn = s_read_set_env("CRANELIFT_SKIP_FN");
    static auto s_only_fn = s_read_set_env("CRANELIFT_ONLY_FN");
    static auto s_dump_fn = s_read_set_env("CRANELIFT_DUMP_FN");
...
#undef M
    ...
    for (size_t i = 0; i < dispatches.size(); ++i) {
        ...
        // Emit continuation instructions for br_table with >8 labels.
        if (dispatches[i].instruction->opcode().value() == Instructions::br_table.value()) {
            ...
            for (size_t base = 8; base < total; base += 8) {
                CraneliftInsn cont {};
                cont.opcode = Cranelift::SYNTHETIC_BR_TABLE_CONT;
