...
#        if ARCH(AARCH64)
                if (offset + sizeof(u32) > code_size)
                    break;
                u32 instruction;
                __builtin_memcpy(&instruction, reinterpret_cast<void const*>(pc), sizeof(instruction));
                if (instruction != 0x0000c11f)
                    break;
#        elif ARCH(X86_64)
                if (offset + 2 > code_size)
                    break;
                auto* instruction = reinterpret_cast<u8 const*>(pc);
