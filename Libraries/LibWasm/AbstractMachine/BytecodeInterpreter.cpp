...
CompiledInstructions try_compile_instructions(Expression const& expression, Span<FunctionType const> functions)
{
    ...
    auto const insert_default_dispatch = [&result](size_t at, Instruction const& instruction) {
        result.dispatches.insert(at, { { .instruction_opcode = instruction.opcode() }, &instruction });
        result.src_dst_mappings.insert(at, { .sources = { Dispatch::Stack, Dispatch::Stack, Dispatch::Stack }, .destination = Dispatch::Stack });
    };
...
#undef M
    ...
    for (auto& call_info : eligible_calls) {
        ...
        insert_default_dispatch(call_info.earliest_arg_index, result.extra_instruction_storage.unsafe_last());
