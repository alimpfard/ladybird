...
ErrorOr<void, ValidationError> Validator::validate(Wasm::TagType const& type)
{
    // The function type t1^n -> t2^m must be valid
    TRY(validate(type.type()));
    // The type sequence t2^m must be empty
    if (!type.type().results().is_empty())
...
VALIDATE_INSTRUCTION(throw_)
{
    ...
    if (!tag_type.type().results().is_empty())
        return Errors::invalid("throw type"sv, "empty"sv, tag_type.type().results());

    for (size_t i = 0; i < tag_type.type().parameters().size(); ++i)
        TRY(stack.take(tag_type.type().parameters()[tag_type.type().parameters().size() - i - 1]));
...
VALIDATE_INSTRUCTION(try_table)
{
    ...
    for (auto& catch_ : args.catches) {
        ...
        if (auto tag = catch_.matching_tag_index(); tag.has_value()) {
            ...
            if (!tag_type.type().results().is_empty())
                return Errors::invalid("catch type"sv, "empty"sv, tag_type.type().results());

            Span<ValueType const> parameters_to_check = tag_type.type().parameters().span();
            if (catch_.is_ref()) {
                // catch_ref x l
                auto& parameters = tag_type.type().parameters();
