...
async fn dispatch(state: &Arc<State>, id: i32, mut decoder: Decoder<'_>) -> Result<()> {
    ...
    if id == M::InitTransport as i32 {
        ...
        let (ok, error_kind, error_message) =
            apply_description(state, req.pc_id, &req.sdp_type, req.sdp, true).await;
        send(
            &state.writer,
        ...
        let (ok, error_kind, error_message) =
            apply_description(state, req.pc_id, &req.sdp_type, req.sdp, false).await;
        send(
            &state.writer,
...
async fn apply_description(
    ...
    let Some(desc) = parsed else {
        return (false, "InvalidAccessError".to_string(), "could not parse SDP".to_string());
    };
