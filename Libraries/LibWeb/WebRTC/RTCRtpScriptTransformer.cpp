...
WebIDL::ExceptionOr<void> RTCRtpScriptTransformer::enqueue_encoded_frame(JS::Value frame)
{
    ...
    TRY(Streams::readable_stream_default_controller_enqueue(controller, frame));
...
void RTCRtpScriptTransformer::initialize(JS::Realm& realm)
{
    WEB_SET_PROTOTYPE_FOR_INTERFACE(RTCRtpScriptTransformer);
