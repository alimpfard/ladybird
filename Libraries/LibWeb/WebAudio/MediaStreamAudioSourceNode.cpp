...
void MediaStreamAudioSourceNode::attach_input_track()
{
    ...
    m_sink->on_frames = [ring = m_ring, conversion_scratch = Vector<float>()](float const* samples, size_t frame_count, u8 channel_count, u32) mutable {
        ...
        if (channel_count == ring_channel_count) {
            push_frames_if_enough_room(*ring, { samples, frame_count * channel_count });
            return;
        }
        ...
        push_frames_if_enough_room(*ring, conversion_scratch);
    };
