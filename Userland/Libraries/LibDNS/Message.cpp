...
String Options::to_string() const
{
    ...
    builder.appendff("QR: {}, Opcode: {}, AA: {}, TC: {}, RD: {}, RA: {}, RCODE: {}",
        is_question() ? "Q" : "R",
...
ErrorOr<size_t> Message::to_raw(ByteBuffer& out) const
{
    ...
    VERIFY(header.additional_count == 0);
...
ErrorOr<ResourceRecord> ResourceRecord::from_raw(ParseContext& ctx)
{
    ...
    auto type = static_cast<ResourceType>(static_cast<u16>(TRY(ctx.stream.read_value<NetworkOrdered<u16>>())));
    auto class_ = static_cast<Class>(static_cast<u16>(TRY(ctx.stream.read_value<NetworkOrdered<u16>>())));
    auto ttl = static_cast<u32>(TRY(ctx.stream.read_value<NetworkOrdered<u32>>()));
    auto rd_length = static_cast<u16>(TRY(ctx.stream.read_value<NetworkOrdered<u16>>()));
    ByteBuffer rdata;
    TRY(ctx.stream.read_until_filled(TRY(rdata.get_bytes_for_writing(rd_length))));
    ...
    ScopeGuard guard([&] {
        ctx.pointers = move(rdata_ctx.pointers);
    });
...
#define PARSE_AS_RR(TYPE)                                                                                                                                   \
    do {                                                                                                                                                    \
        ...
        return ResourceRecord { move(name), type, class_, ttl, rr };                                                                                        \
    } while (0)
    ...
    //     PARSE_AS_RR(TLSA);
    default:
        return ResourceRecord { move(name), type, class_, ttl, move(rdata) };
    }
#undef PARSE_AS_RR
}
